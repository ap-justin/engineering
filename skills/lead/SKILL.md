...
- Keep a builder's run bounded. A subagent runs in its own context and can't be capped mid-run, so scope in, don't cap after: a build that would touch many files/subsystems gets **split across sequential builders** (or routed through `planner`'s tracer-bullet slices), not handed to one builder that sprawls to hundreds of K tokens. "Sequential builders" means *sequential fresh agents* — a new `Agent` call per slice, not `SendMessage` to the last one (see *One task, one builder*). Isolation already keeps that bloat out of your context — this keeps it out of the builder's.
- **One task, one builder — a new task starts fresh.** `SendMessage`/resume reuses a builder's context *intact*, so it's only for continuing the **same** task: folding in that slice's review findings, applying an amendment to it, running its fix loop. Never route the next slice or an unrelated task into a builder that's already carrying one — its 150k of prior context re-loads *on top of* the new work and the run balloons (150k + 150k ⇒ 300k). Each independent unit of work is a **fresh `Agent` call** with its own scoped context; the warm context you'd save by reusing is exactly the bloat isolation exists to shed. And cap the same-task reuse too: a slice that needs more than **2 fix-loop rounds** stops growing one builder — surface the remainder and re-slice into a fresh run (matches Step 4's "max 2 loops").
